...
]=====> KASIH KREDIT GW RYU AJG<=====[
]=====> GANTI GANTI AJA GW BOLEHIN <=====[
FREE RECODE SAYANGKU
JNG LUPA SEWA BOT GUA AJG
Dan semua creator bot wa tanpa kecuali
MHANK BAR BAR
DLL 
TQ TQ BANYAK BANYAK

//NOTE RYU CUMA RECODE SC BY MHANK BAR BAR
...
*/
const ownerNumber = ["6288239430315@s.whatsapp.net","6288239430315@s.whatsapp.net"]
const botName = 'RYUBOT'
const ownerName = 'RYUBOT'
const BarBarKey = 'tempel sini' // APIKEY BELI DI DAFFAGANZ
const VhtearKey = 'tempel sini' // beli di 088239430315
const XteamKey = 'tempel sini' // APIKEY XTEAM BELI SENDIRI:V
const TobzKey = 'Bidmzz1sJ2L1TKyqaMEU' // PUNYA  DAFFAGANZZ
const ApiDevolover = 'PunyaDfaGanz'
/*
]=====> SC BY RYUBOT <=====[
*/
//SUBREK JUGA  BELUM PUNYA YT:V
//WA GW 088239430315
/*       
]=====> LOAD JSON <=====[
*/
...
const daffa = new WAConnection()
	daffa.logger.level = 'warn'
	console.log(banner.string)
	daffa.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' POWERED BY RYUBOT'))
	})
	daffa.on('credentials-updated', () => {
		fs.writeFileSync('./session.json', JSON.stringify(daffa.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'Login Info Updated')
	})
	fs.existsSync('./session.json') && daffa.loadAuthInfo('./session.json')
	daffa.on('connecting', () => {
...
ikyy.on('group-participants-update', async (anu) => {
		...
		try {
			const mdata = await daffa.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = await daffa.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `[ *WELCOME IN GC ${mdata.subject}* ] \n___________________________\n@${num.split('@')[0]} Intro/Dikick!!! \n➸ Nama : \n➸ Umur : \n➸ Askot : \n➸ Gender : \n➸ Udah Punya Doi/Blm: \n➸ Pap Muka dumlu!!! \n➸ Instagram? \n𝐒𝐚𝐯𝐞 𝐍𝐨𝐦𝐨𝐫 𝐀𝐃𝐌𝐈𝐍! \n *___________________________*\nJangan jadi kutu lomcat sayang!!`
				let buff = await getBuffer(ppimg)
				daffa.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await daffa.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
...
ikyy.on('group-participants-update', async (anu) => {
	...
	try {
		...
		if (anu.action == 'add') {
			...
			teks = `Hallo @${num.split('@')[0]}\n Welcome to group ${mdata.subject} Jangan lupa salam kau!...Betah-betah yah di sini🖤`
			let buff = await getBuffer(ppimg)
			daffa.sendMessage(mdata.id, teks, MessageType.text)
		} else if (anu.action == 'remove') {
			num = anu.participants[0]
			try {
			...
			teks = `Sayonara🥳 @${num.split('@')[0]} bacain doa buat yang left/terkick!.`
			let buff = await getBuffer(ppimg)
...
ikyy.on('message-update', async (hurtz) => {
	try {
		...
		if (messageStubType == 'REVOKE') {
			...
			const conts = hurtz.key.fromMe ? RYU.user.jid : RYY.contacts[sender] || { notify: jid.replace(/@.+/, '') }
			const pushname = hurtz.key.fromMe ? RYU.user.name : conts.notify || conts.vname || conts.name || '-'
			const opt4tag = {
...
break
				...
				case 'donate':
					...
					ikyy.sendMessage(from, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_「 のR Y U B O T あ」_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')}}}})

break
					case 'iklan':
					ikyy.sendMessage(from, iklan(pushname, prefix, botName, ownerName), text)
					ikyy.sendMessage(from, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_「 の R Y U B O T あ」_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')}}}})
...
break 
				...
				if (args.length < 1) return reply('Teksnya mana kak? Contoh : ${prefix}nulis1 DAFFA baik hati')
				IKY = body.slice(8)
				reply('[❕] Sedang di proses.........')
				buff = await getBuffer(`https://api.xteam.xyz/magernulis2?text=${IKY}&APIKEY=${XteamKey}`)
				ikyy.sendMessage(from, buff, image, {quoted: mek, caption: 'Lebih baik nulis sendiri ya kak JANGAN MALES :*'})
				await limitAdd(sender)
				ikyy.sendMessage(from, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_「 の R Y U B O T あ」_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')}}}})

break
//FITUR TAMBAHAN DAFFA GANZZ 
case 'tahta':
				if (!isRegistered) return reply(ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (isLimit(sender)) return reply(ind.limitend(pusname))
...
case 'fftourserti':
                ...
                ikyy.sendMessage(from, buffer, image, { quoted: mek, caption: '*Nih kak.. jangan lupa MKS SAMA DAFFA GANZZ*' })
                break
            case 'fftourserti2':
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
            ...
            case 'fftourserti4':
                ...
                ikyy.sendMessage(from, buffer, image, { quoted: mek, caption: '*Nih kak.. jangan lupa mks sama DAFFA GANZ*' })
                break
            case 'fftourserti5':
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                ...
                ikyy.sendMessage(from, buffer, image, { quoted: mek, caption: '*Nih kak.. jangan lupa mks sams DAFFA GANZ*' })
                break
            case 'pubgtourserti':
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
